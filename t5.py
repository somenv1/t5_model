...
train_dataset = load_dataset('squad', split='train')#.select(range(10000))
valid_dataset = load_dataset('squad', split='validation')#.select(range(2000))
...
def encode(example,
    ...
    #question_plus = f"quiz_me: "
    #question_plus += ', '.join([i for i in list(answer)])
    #question_plus += f" context: {str(context)} </s>"
    #answer_plus = ', '.join([i for i in list(answer)])

    question_plus = f"context: {str(context)} </s>"
    answer_plus = f"question:{str(question)} answer:{', '.join([i for i in list(answer)])} </s>"
